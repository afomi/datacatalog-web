namespace :demo do
  ...
  def create_submissions
    @contact1 = ContactSubmission.create!(
      :user_id  => @louis.id,
      :comments => "Help! I lost my password."
    )
    @contact2 = ContactSubmission.create!(
      :name     => "Orly Taitz",
      :email    => "orly@taitz.com",
      :comments => "Check his birth certificate... He's really Kanyan!"
    )
    ...
    @suggestion1 = DataSuggestion.create!(
      :user_id  => @louis.id,
      :title    => "LOUISdb API",
      :url      => "http://www.louisdb.org/api/index.php",
      :comments => "Here's a great API with a wonderful name."
    )
    @suggestion2 = DataSuggestion.create!(
      :name     => "Jeff Corwin",
      :email    => "jeff@corwin.com",
      :title    => "Bat Data",
      :comments => "Would love to see more animal-related data."
    )
  ...
  def create_downloads
    @csv = DataCatalog::Download.create(
      ...
      :preview   => %{
        "title","firstname","middlename","lastname","name_suffix","nickname","party","state","district","in_office","gender","phone","fax","website","webform","email","congress_office","bioguide_id","votesmart_id","fec_id","govtrack_id","crp_id","eventful_id","twitter_id","congresspedia_url","youtube_url","official_rss","senate_class","birthdate"
  ...
  def create_comments
    ...
    DataCatalog.with_key(@louis.api_key) do
      @comments[0] = DataCatalog::Comment.create(
        :source_id => @sunlight_labs_api.id,
        :text      => "This is a great API!"
      )
    end
    DataCatalog.with_key(@david.api_key) do
      @comments[1] = DataCatalog::Comment.create(
        :parent_id => @comments[0].id,
        :source_id => @sunlight_labs_api.id,
        :text      => "Disagree... Had some issues with the XML version..."
      )
    end
    DataCatalog.with_key(@luigi.api_key) do
      @comments[2] = DataCatalog::Comment.create(
        :source_id => @sunlight_labs_api.id,
        :text      => "Yeah, it rocks."
      )
    end
    DataCatalog.with_key(@david.api_key) do
      @comments[3] = DataCatalog::Comment.create(
    ...
    DataCatalog.with_key(@louis.api_key) do
      @comments[4] = DataCatalog::Comment.create(
        :parent_id => @comments[3].id,
        :source_id => @sunlight_labs_api.id,
        :text      => "Nice argumentation there, bud."
      )
    end
  end
